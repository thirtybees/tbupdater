(function () {
  function initializeUpgrader() {
    ...
    function setConfig(key, value, type) {
      $.ajax({
        type: 'POST',
        url: window.upgrader.currentIndex + '&ajax=1&action=setConfig',
        dataType: 'json',
        data: {
          configKey: key,
          configValue: value,
    ...
    function afterUpdateConfig(res) {
      ...
      $upgradeNow.unbind();
      $upgradeNow.replaceWith('<a class="button-autoupgrade" href="' + window.upgrader.currentIndex + '">Click to refresh the page and use the new configuration</a>');
    }
